...
const MINE_CYCLE_MS = 2500 // 矿场每挖出一次 灵石 的周期时长
const MINE_BONUS_GOLD_CHANCE = 0.12 // 每个挖矿周期额外经济奖励的概率
// 注：挖矿获得宝箱的概率随等级下降，见 chest.ts 的 mineChestChance()。
...
export interface BattleState {
  ...
  mined?: number // 矿场：本次累计挖到的 灵石
  minePop?: FightHitFx | null // 矿场：最近一次挖矿产出（用于飘字动画）
}
...
export const useGame = create<GameState>((set, get) => {
  ...
  // -- 授予一个宝箱（存入宝箱库存，等待玩家在「宝箱」面板手动开启）--
  const grantChest = (tier: ChestTierKey, lv: number) => {
    const chest: ChestInstance = { id: uid('chest'), tier, lv: Math.max(1, Math.floor(lv)) }
    set({ chests: [...get().chests, chest] })
    pushLog({ type: 'trophy', msg: `获得${CHEST_TIERS[tier].name}！可在「宝箱」中开启。` })
  }

  // -- 结算一组开箱奖励：发放灵石 / 塞入背包（满则折算灵石），返回汇总供 UI 与日志使用 --
  const applyChestRewards = (rolled: ChestReward[]) => {
    const finalRewards: ChestReward[] = []
    const logItems: Item[] = []
    let goldSum = 0
    for (const rw of rolled) {
      if (rw.kind === 'gold') {
        get().addGold(rw.amount)
        goldSum += rw.amount
        finalRewards.push(rw)
      } else {
        const ok = addItemToBackpack(rw.item)
        if (ok) {
          finalRewards.push(rw)
          logItems.push(rw.item)
        } else {
          const g = sellPrice(rw.item)
          get().addGold(g)
          goldSum += g
          finalRewards.push({ kind: 'gold', amount: g, converted: true })
        }
      }
    }
    const hasUnique = finalRewards.some((rw) => rw.kind === 'item' && rw.item.quality.name === '独特')
    return { finalRewards, logItems, goldSum, hasUnique }
  }
  ...
  const battleStep = () => {
    ...
    // ---- 矿场：无风险挖矿，进度条循环，每周期产出微薄 灵石，不掉血 ----
    if (b.dungeon.type === 'mine') {
      const step = 100 / (MINE_CYCLE_MS / BATTLE_TICK_MS)
      let left = b.left + step
      let mined = b.mined ?? 0
      let minePop = b.minePop ?? null
      if (left >= 100) {
        left = 0
        const gained = mineYield(s.lv)
        get().addGold(gained)
        mined += gained
        minePop = { dmg: gained, id: ++fxSeq }
        // 额外经济：有几率一次挖到可观的额外灵石（3~6 倍）
        if (Math.random() < MINE_BONUS_GOLD_CHANCE) {
          const bonus = gained * (3 + Math.floor(Math.random() * 4))
          get().addGold(bonus)
          mined += bonus
          minePop = { dmg: gained + bonus, id: ++fxSeq }
        }
        // 低概率挖到宝箱（概率随等级下降：lv1≈10% → lv100+≈0.01%）
        if (Math.random() < mineChestChance(s.lv)) grantChest(mineChestTier(), s.lv)
      }
      set({ battle: { ...b, left, mined, minePop } })
      return
    }
  ...
  return {
    ...
    selectMine: () => {
      set({ selectedDungeon: createMineDungeon(), reChallenge: false })
    },
    ...
    beginBattle: () => {
      ...
      } else if (dungeon.type === 'mine') {
        pushLog({ type: 'warning', msg: '来到矿场，开始当矿奴挖矿…（安全无风险）' })
      } else {
        pushLog({ type: 'warning', msg: `进入 ${dungeon.type === 'endless' ? `无尽（第${dungeon.lv}层）` : dungeon.name}` })
      }
      set({
        battle: { dungeon, left: 0, nextEvent: 1, phase: 'move', fightUntil: 0, lastResult: null, fight: null, mined: 0, minePop: null },
      })
      startTimer()
    },
    ...
    stopBattle: (manual) => {
      ...
      set({ battle: null, selectedDungeon: null })
      if (b?.dungeon.type === 'mine') {
        pushLog({ type: 'trophy', msg: `收工！本次挖矿共获得灵石 ${fmtNum(b.mined ?? 0)}。` })
      } else if (manual) {
        pushLog({ type: 'warning', msg: '手动中断了挑战。' })
      }
    },
    ...
    openChest: (id) => {
      ...
      set({ chestReveal: { tier: chest.tier, rewards: finalRewards } })
    ...
    openTen: (tier) => {
      ...
      set({ chestReveal: { tier, rewards: finalRewards, pulls: TEN_PULL } })
    ...
    dismissChestReveal: () => set({ chestReveal: null }),
    ...
    loadGame: (raw) => {
      try {
        ...
        set({
          ...
          chests: normalizeChests(d.chests),
        })
        get().recompute(false)
        setMp(get().maxMp)
...
/** 校验宝箱库存，丢弃等级键非法或缺失 lv 的损坏条目（防手改/旧版存档导入崩溃）。 */
function normalizeChests(raw: unknown): ChestInstance[] {
  if (!Array.isArray(raw)) return []
  const out: ChestInstance[] = []
  for (const c of raw) {
    const o = c as ChestInstance
    if (o && typeof o === 'object' && CHEST_TIER_KEYS.has(o.tier) && typeof o.lv === 'number') {
      out.push({ id: typeof o.id === 'string' ? o.id : uid('chest'), tier: o.tier, lv: o.lv })
    }
  }
  return out
}
